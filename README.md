...
- `store` 모드의 메뉴 주문, 좌석 예약 및 웨이팅 요청
...
관리자 브라우저에서 JPEG, PNG, WEBP만 허용하고 원본 2MB, 긴 변 800px,
출력 품질 0.8 기준으로 리사이징합니다. 서버에서도 Base64 디코딩 예상 크기가
2MB를 넘으면 요청을 거절합니다. 수정 시 새 이미지를 선택하지 않으면 기존
이미지를 유지합니다.
